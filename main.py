...
def manga(id, cursor):
	...
	if res is not None:
		...
		if session:
			...
			cursor.execute('select * from hentai_user where id_hentai=? and id_user=?;', (id, get_session(request)[1])).fetchone() is not None:
...
def show(id, page, cursor):
	...
	args = ['', '/show/'+str(id)+'/0', '/show/'+str(id)+'/'+str(page-1), page + 1, l_manga, '',
		'/show/'+str(id)+'/'+str(page+1), '/show/'+str(id)+'/'+str(l_manga - 1)]
	if page == 0:
